...
<div id="div_nueva_reserva">
		...
		<?php  include('pantalla_captura_total.php') 
		//include('pregunte_asesor.php') 
	 ...
	 /////////////////////////////
						  ...
						  //$("#replica").val($("#mitexto").val());
							$.post('buscar_cliente.php',data,function(a){
												//$(window).attr('location', '../index.php);
								$("#div_buscar_cliente").html(a);					//alert(data);
							});	
   						});
//////////////////////////////////////////////////////////	
...
////////////////////////////////////////////////////////				 
	$("#btn_grabar_reserva").click(function(){
					 	...
					 	//////////////////////
					      ...
					      $(no_contrato).focus();
					         ...
					         //numero_radicado = $("#no_radicado").val();
					         //alert('numero radicado ' + numero_radicado);
					        if($("#no_radicado").val().length < 1)
					        { alert('digite datos radicado reserva o negocio ');
					      $(no_radicado).focus();
					          return false;
					         }
					    ...
					    //////////////////////
							...
							*/
					        ...
					        return false; }
					        /*
					        if($("#tours_incluidos").val().length < 1)
					        { alert('Digite tours_incluidos'); 
					        $(tours_incluidos).focus();
					        return false; }
							
					        if($("#contacto_asistencia").val().length < 1)
					        { alert('Digite contacto_asistencia'); 
					        $(contacto_asistencia).focus();
					        return false; }
					       ...
					       */
					        ...
					        return false; }
					          ...
					          if($("#vr_efectivo").val().length < 1)
					        { alert('Digite valor  efectivo'); 
					        $(vr_efectivo).focus();
					        return false; }

					           if($("#vr_cons_trans").val().length < 1)
					        { alert('Digite valor  consignacion /transferencia'); 
					        $(vr_cons_trans).focus();
					        return false; }

					             if($("#vr_tarjeta").val().length < 1)
					        { alert('Digite valor  tarjeta debito/credito'); 
					        $(vr_tarjeta).focus();
					        return false; }
					 	...
					 	//////////////////////
							...
							data += '&no_contrato=' + $("#no_contrato").val();
  ...
  //////////////////
    ...
    function  iniciar_valores(){
		...
		///////////////////////////
		 $("#ciudad_destino").change(function(event){
		 	//alert('selecciono un destino ');
		 	
            var id = $("#ciudad_destino").find(':selected').val();
            $("#id_tarifa").load('genera_select_tarifa.php?id='+id);
