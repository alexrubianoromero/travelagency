...
<body>
 <a href="mostrar_informe_proveedores_excell.php?id_proveedor=<?php echo $_REQUEST['id_proveedor'] ?>&fecha_in=<?php echo $_REQUEST['fecha_in']; ?>&fecha_fin=<?php echo $_REQUEST['fecha_fin']; ?>&id_concepto=<?php echo $_REQUEST['id_concepto']; ?>"  > ENVIAR EXCEL</a>
 ...
 echo  '<thead>'; 
  ...
  while($cxp_proveedores = mysql_fetch_assoc($con_cxp_proveedores))
  {
        ...
        echo '<td>'.$datos_concepto['nombre_concepto'].'</td>';
          ...
          echo '<td align="right">'.number_format($cxp_proveedores['valor_total'], 0, ',', '.') .'</td>';
          echo '<td align="right">'.number_format($arr_suma_recibos['valor'], 0, ',', '.') .'</td>';
         echo '<td align="right">'.number_format($cxp_proveedores['saldo'], 0, ',', '.') .'</td>';
  }	
echo '</tbody>';
